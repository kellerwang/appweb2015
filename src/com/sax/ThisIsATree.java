...
public class ThisIsATree {
	...
	public void createTree(){
				...
				for(int iB = 1;iB<5;iB++){
					for(int iC = 1;iC<5;iC++){
						for(int iD = 1;iD<5;iD++){
							nodeList.add(new Node(array[iB]+array[iC]+array[iD]));
						}
					}
				}
			}
					...
					for(int iC = 1;iC<5;iC++){
						for(int iD = 1;iD<5;iD++){
							for(int iE = 1;iE<5;iE++){
								nodeList.add(new Node(array[iB]+array[iC]+array[iD]+array[iE]));
							}
						}
					}
				}
		...
		for(int parentIndex = 0;parentIndex<85;parentIndex++){
			nodeList.get(parentIndex).childA = nodeList.get(parentIndex*4+1);
			nodeList.get(parentIndex).childB = nodeList.get(parentIndex*4+2);
			nodeList.get(parentIndex).childC = nodeList.get(parentIndex*4+3);
			nodeList.get(parentIndex).childD = nodeList.get(parentIndex*4+4);
		}
	}
	/*
	 * 前序遍历
	...
	public static void main(String[] args){
		ThisIsATree tree = new ThisIsATree();
		tree.createTree();
		
		Node root = nodeList.get(0);
		nodeList.get(86).setValue(10);
		//nodeList.get(86).setValue(1000);
