...
public class SymbolCreater {
	 ...
	 * 符号化
	 */
	private static void createSymbol(ThisIsATree tree, String filename,
			int slidesize, int symbolsize, Double[] border) throws IOException {
		...
		for (int i = 0; i < line - windowsize; i += slidesize) {
			...
			for (int j = 0; j < 4; j++) {
				...
				if (average[j] < border[1]) {
					if (average[j] < border[0]) {
						...
						temp2 = temp2 + "a,";
						// temp1 =
						// temp1+Double.toString(average[j])+"-a"+"\r\n";
					} else {
						temp1 = temp1 + "b";
						temp2 = temp2 + "b,";
						// temp1 =
						// temp1+Double.toString(average[j])+"-b"+"\r\n";
					}
				} else {
					if (average[j] < border[2]) {
						...
						temp2 = temp2 + "c,";
						// temp1 =
						// temp1+Double.toString(average[j])+"-c"+"\r\n";
					} else {
						temp1 = temp1 + "d";
						temp2 = temp2 + "d,";
						// temp1 =
						// temp1+Double.toString(average[j])+"-d"+"\r\n";
					}
				}
				sum = 0;
			}
			...
			// 将滑窗编号存入对应叶节点
			tree.nodeList.get(nodeNum).setValue(
		...
		FileOutputStream fos1 = new FileOutputStream("./data/" + filename
				+ "-symbol.txt");
	 ...
	 * 文件夹下所有txt文件读取
	 */
	public static boolean readfile(List<Double> num, String filepath)
	...
	public static boolean readfile2(ThisIsATree tree, int slidesize,
			int symbolsize, Double[] border, String filepath)
			throws FileNotFoundException, IOException, InterruptedException {
		try {
			String filename = "";
			File file = new File(filepath);
				...
				for (int i = 0; i < filelist.length; i++) {
					...
					if (!tempfile.isDirectory()) {
						...
						System.out.println("absolutepath="
								+ tempfile.getAbsolutePath());
						System.out.println("name=" + tempfile.getName());
						createSymbol(tree, filename, slidesize, symbolsize,
								border);// 符号化
	...
	private static void symbolData(String filename, List<Double> num)
			throws IOException {
		File file = new File("./data/" + filename);
		BufferedReader reader = null;
		String templist[];
